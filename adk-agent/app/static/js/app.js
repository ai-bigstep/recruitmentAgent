...
function connectWebsocket() {
  ...
  websocket.onmessage = function (event) {
    ...
    console.log("message_from_server.interrupted   ",message_from_server)
if (message_from_server?.interrupted === true && !message_from_server?.turn_complete) {
...
// Audio recorder handler with interruption detection
let lastAudioTimestamp = Date.now();
const INTERRUPTION_THRESHOLD = 2000; // 2 seconds of silence to trigger interruption

function audioRecorderHandler(pcmData) {
  const currentTime = Date.now();
  
  // Check for interruption (silence)
  if (currentTime - lastAudioTimestamp > INTERRUPTION_THRESHOLD) {
    // Send interruption signal
    sendMessage({
      mime_type: "text/plain",
      data: "[INTERRUPTION_DETECTED]"
    });
  }
